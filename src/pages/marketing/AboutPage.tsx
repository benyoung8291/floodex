...
const values = [
  { icon: Zap, title: 'Speed', desc: 'Time is money in restoration. We build tools that save hours, not minutes.', bg: 'bg-primary/5' },
  { icon: Shield, title: 'Reliability', desc: 'Your data is critical. We ensure 99.9% uptime and secure cloud storage.', bg: 'bg-accent/5' },
  { icon: Users, title: 'Partnership', desc: "We're not just a vendor — we're your technology partner in restoration.", bg: 'bg-primary/5' },
  { icon: Lightbulb, title: 'Innovation', desc: 'We constantly evolve based on feedback from real restoration pros.', bg: 'bg-accent/5' },
];
    ...
    <MarketingLayout>
      ...
      <section className="px-4 md:px-8 pt-16 md:pt-24 pb-20 md:pb-28 max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05, duration: 0.5 }}>
          <div className="text-xs font-extrabold uppercase tracking-[0.12em] text-muted-foreground mb-4">About FloodEx</div>
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12, duration: 0.5 }} className="text-[clamp(44px,7vw,88px)] font-black leading-[0.98] tracking-[-0.04em] text-foreground max-w-[850px] mb-7">
          ...
          <span className="bg-accent text-white px-3 py-1 inline-block rounded-xl -rotate-1 my-1">for restoration pros</span>
        </motion.h1>
      ...
      <section className="mx-4 md:mx-8 rounded-[32px] bg-secondary px-6 md:px-12 py-20 md:py-28">
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
          <AnimateIn>
            <div className="text-xs font-extrabold uppercase tracking-[0.12em] text-muted-foreground mb-4">Our mission</div>
            ...
            <p className="text-[15px] text-muted-foreground leading-[1.7] font-medium mb-6">
            ...
            <p className="text-[15px] text-muted-foreground leading-[1.7] font-medium">
          ...
          <AnimateIn>
            <div className="bg-white border border-border/40 rounded-3xl p-8">
              <h3 className="font-extrabold text-[17px] mb-4 text-foreground">Operated by</h3>
              <p className="text-muted-foreground font-medium mb-2">Local Carpet Cleaning Pty Ltd</p>
              <p className="text-sm text-muted-foreground/70">ABN 15 682 871 192</p>
              <div className="mt-6 pt-6 border-t border-border/40">
                <p className="text-[13px] text-muted-foreground leading-[1.7] font-medium">
      ...
      <section className="px-4 md:px-8 py-20 md:py-28 max-w-6xl mx-auto">
        <AnimateIn>
          <div className="text-xs font-extrabold uppercase tracking-[0.12em] text-muted-foreground mb-4">Our story</div>
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { step: '1', title: 'The Problem', desc: 'Paper forms, manual calculations, and hours spent creating reports.', bg: 'bg-primary/5' },
            { step: '2', title: 'The Insight', desc: 'Every tech has a smartphone — why not use it for documentation?', bg: 'bg-accent/5' },
            { step: '3', title: 'The Solution', desc: 'A mobile-first platform purpose-built for water damage restoration.', bg: 'bg-primary/5' },
          ].map((item) => (
            <StaggerItem key={item.step}>
              <div className={`${item.bg} border border-border/40 rounded-3xl p-8 h-full`}>
                <div className="w-9 h-9 rounded-full bg-foreground text-white text-[13px] font-black flex items-center justify-center mb-6">
                  {item.step}
                </div>
                <h4 className="text-[17px] font-extrabold tracking-tight mb-3 text-foreground">{item.title}</h4>
      ...
      <section className="mx-4 md:mx-8 rounded-[32px] bg-secondary px-6 md:px-12 py-20 md:py-28">
        <div className="max-w-5xl mx-auto">
          <AnimateIn>
            <div className="text-xs font-extrabold uppercase tracking-[0.12em] text-muted-foreground mb-4">Our values</div>
            <h2 className="text-[clamp(34px,4.5vw,56px)] font-black leading-[1.0] tracking-[-0.04em] text-foreground mb-14">What we stand for</h2>
          </AnimateIn>
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {values.map((v) => (
              <StaggerItem key={v.title}>
                <div className={`${v.bg} border border-border/40 rounded-3xl p-7 flex flex-col gap-4 h-full hover:shadow-lg transition-shadow`}>
                  <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center">
                    <v.icon className="w-6 h-6 text-foreground" />
                  </div>
                  <h4 className="text-[17px] font-extrabold tracking-tight text-foreground">{v.title}</h4>
                  <p className="text-[13px] text-foreground/60 leading-[1.6] font-medium">{v.desc}</p>
                </div>
              </StaggerItem>
            ))}
          </StaggerContainer>
      ...
      <div className="mx-4 md:mx-8 my-8 rounded-[32px] bg-foreground px-6 md:px-12 py-20 md:py-28 text-center">
        <AnimateIn>
          <h2 className="text-[clamp(34px,5vw,60px)] font-black leading-[1.0] tracking-[-0.04em] text-white mb-4">
            Ready to<br /><span className="text-accent">get started?</span>
          </h2>
