-- Pin search_path on the pgmq helper functions
...
-- Only the backend (service_role) may touch the email queue plumbing
...
-- Billing predicate: signed-in users need it for RLS; anonymous visitors do not
REVOKE ALL ON FUNCTION public.tenant_billing_active(uuid) FROM anon, public;
GRANT EXECUTE ON FUNCTION public.tenant_billing_active(uuid) TO authenticated, service_role;

-- Tenant provisioning trigger must never be callable from the API
REVOKE ALL ON FUNCTION public.handle_new_user() FROM anon, authenticated, public;
REVOKE ALL ON FUNCTION public.accept_team_invitation(text, uuid) FROM anon, public;
